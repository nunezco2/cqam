...
impl Kernel for PhaseShift {
    ...
    ///
    /// Computes angle = self.amplitude.norm(), then constructs U as a
    /// dim x dim diagonal matrix with U[k][k] = C64::exp_i(angle * k),
    /// and delegates to DensityMatrix::apply_diagonal_unitary.
    fn apply(&self, input: &DensityMatrix) -> Result<DensityMatrix, CqamError> {
        let rate = self.amplitude.norm();
        let dim = input.dimension();
        let phases: Vec<_> = (0..dim).map(|k| C64::exp_i(rate * (k as f64))).collect();
    ...
    fn apply_sv(&self, input: &Statevector) -> Result<Statevector, CqamError> {
        ...
        let result_amps: Vec<_> = if dim >= PAR_THRESHOLD {
            amps.par_iter().enumerate().map(|(k, &amp)| {
                let phase = C64::exp_i(rate * (k as f64));
                phase * amp
            }).collect()
        } else {
