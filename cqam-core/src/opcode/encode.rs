...
pub fn encode(instr: &Instruction, label_map: &HashMap<String, u32>) -> Result<u32, CqamError> {
    match instr {
        ...
        Instruction::IMov { dst, src } => encode_rr(op::IMOV, *dst, *src),
        Instruction::FMov { dst, src } => encode_rr(op::FMOV, *dst, *src),
        Instruction::ZMov { dst, src } => encode_rr(op::ZMOV, *dst, *src),
