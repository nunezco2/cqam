...
fn test_ildi_negative() {
    ...
    assert_eq!(ctx.iregs.get(0), -32768);
}
...
fn test_fadd_fsub() {
    ...
    assert!((ctx.fregs.get(2) - 4.0).abs() < 1e-10);

    execute_instruction(&mut ctx, &Instruction::FSub { dst: 3, lhs: 1, rhs: 0 }).unwrap();
    assert!((ctx.fregs.get(3) - 1.0).abs() < 1e-10);
}
...
fn test_fmul_fdiv() {
    ...
    assert!((ctx.fregs.get(2) - 12.0).abs() < 1e-10);

    execute_instruction(&mut ctx, &Instruction::FDiv { dst: 3, lhs: 1, rhs: 0 }).unwrap();
    assert!((ctx.fregs.get(3) - (4.0 / 3.0)).abs() < 1e-10);
}
...
fn test_cvtif() {
    ...
    assert!((ctx.fregs.get(0) - 42.0).abs() < 1e-10);
}
...
fn test_cvtfz() {
    ...
    assert_eq!(ctx.zregs.get(0), (5.0, 0.0));
}

#[test]
fn test_cvtzf() {
    let mut ctx = ExecutionContext::new(vec![]);
    ctx.zregs.set(0, (3.125, 2.625));
